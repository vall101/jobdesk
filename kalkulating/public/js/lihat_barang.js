...
document.addEventListener("DOMContentLoaded", function () {
  let map = L.map('map').setView([-7.4245, 109.2449], 14);
    let marker;
    let koordinat;
    ...
    map.on('click', async function (e) {
      ...
      try {
        ...
        const jalan = address.road || address.pedestrian || '';
        const gang = address.neighbourhood || '';
        const kecamatan = address.city_district || address.county || '';
        const kabupaten = address.city || address.town || address.village || '';
        const provinsi = address.state || '';

        const alamatLengkap = [jalan, gang, kecamatan, kabupaten, provinsi]
          .filter(Boolean).join(', ');
